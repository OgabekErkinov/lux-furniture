...
const GiveQuestionModal = ({toggleModal}) => {
    ...
    const sendMessage = (e) => {
        ...
        if (!(phoneNumber.length === 13 || questionText.length > 5 || name.length > 3)) {
            alert(modalTexts.errorFill);
        } else {
            axios
                .post(url, {
                    chat_id: myId,
                ...
                .catch((error) => {
                    alert(`${modalTexts.errorSend} ${error.message}`);
                });
        }
    };
  ...
  return (
    <Box height='100vh' width='100vw' position='fixed' zIndex={10} top='0' >
        <Stack height='100%' width='100%'>
            ...
            <Box height='90vh' width='100%' display='flex' justifyContent='center' bgcolor='#ffffff'>
