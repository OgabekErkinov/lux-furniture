...
const DownloadSection = () => {
    ...
    const handleChangeNumber = (string) => {
        setNumber(`+998 ${string.split('').slice(0,8).join('')}`)
        
    }

    const handleClick = () => {
      (number.split('').slice(4,8).join('') === '+998' || number.length !== 13) ?  setIsNumber(false) : setIsNumber(true)
      console.log(isNumber, number, number.length)
      console.log(typeof number)
      console.log(number.split('').slice(4,8).join())
