...
const Languages = () => {
  ...
  return (
    <Box display='flex' justifyContent='center' gap='5px' height='100%' alignItems='flex-end'>
            {
              languages.map((language, idx) => {
                if(language.name !== currentLanguage){
                return (
                          <Box component='img' src={language.flag} height={smScreen ? '1.5rem' : mdScreen ? '2rem' : '2.5rem'} 
                                                                   width={smScreen ? '2rem' : mdScreen ? '2.5rem' : '3rem'} 
