...
const Languages = () => {
  ...
  return (
    <Box display='flex' height={smScreen ? '1rem' : mdScreen ? '2rem' : '3rem'} width={smScreen ? '2rem' : mdScreen ? '3rem' : '4rem'}>
            {
              languages.map((language, idx) => {
                if(language.name !== currentLanguage){
                return (
                    <Button key={idx} onClick={()=>handleLanguage(language.name)} sx={{height : '100%', width : '4rem'}}>
